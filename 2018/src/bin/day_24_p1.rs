...
fn sim_iter(infection_groups: &mut Vec<Group>, immune_groups: &mut Vec<Group>) {
...
fn main() {
    ...
    while !infection_groups.is_empty() && !immune_groups.is_empty() {
        sim_iter(&mut infection_groups, &mut immune_groups);
    }

    let ans: i32 = if !infection_groups.is_empty() {
        infection_groups.iter().map(|x| x.units).sum()
    } else {
