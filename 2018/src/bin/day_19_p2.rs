...
fn main() {
    ...
    while ptr < lines.len() - 1 {
        ...
        // By manually analyzing instructions 3-11 (which are part of a hot loop)
        // and replacing the logic with an optimized computation.
        // NOTE: this might not work for all inputs
        if ptr == 11 {
            reg[5] = 1;
            if reg[3] % reg[4] == 0 {
                let val = reg[3] / reg[4];
