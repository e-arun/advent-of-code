...
const parseLine = (line: string): Instr => {
	...
	if (words[5] === "output") {
		low = -low - 1; // extra -1 to disambiguate between bot 0 and output 0
	}

	let high = Number(words[11]);
	if (words[10] === "output") {
		high = -high - 1; // extra -1 to disambiguate between bot 0 and output 0
	}
...
mainLoop: while (true) {
	for (const instr of instrArr) {
		...
		if (botChips.has(-1) && botChips.has(-2) && botChips.has(-3)) {
			const ans =
				botChips.get(-1)![0] * botChips.get(-2)![0] * botChips.get(-3)![0];
			console.log(ans);
			break mainLoop;
		}
	}
